...
===================================================================================== */
#ifndef __TCHEM_ISOTHERMAL_TRANSIENT_CONT_STIRRED_TANK_REACTOR_HPP__
#define __TCHEM_ISOTHERMAL_TRANSIENT_CONT_STIRRED_TANK_REACTOR_HPP__

#include "TChem_KineticModelData.hpp"
#include "TChem_Util.hpp"

#include "TChem_Impl_IsothermalTransientContStirredTankReactor.hpp"
...
struct IsothermalTransientContStirredTankReactor
{
    ...
    const RealType1DViewType& vals)
  {
    ...
    Kokkos::parallel_for(
      ...
      [&](const ordinal_type& i) {vals(i) = Ys(i);});

    const ordinal_type n(Zs.extent(0));

    Kokkos::parallel_for(
      Kokkos::TeamVectorRange(member, n),
      [=](const ordinal_type& i) { vals(i + m  ) = Zs(i); });
    ...
    const Tines::value_type_1d_view<real_type,DeviceType>& Zs)
  {
    ...
    Kokkos::parallel_for(
      Kokkos::TeamVectorRange(member, n),
      [=](const ordinal_type& i) { Zs(i) = vals(i + m); });
    member.team_barrier();
  }
    ...
    const KineticSurfModelConstData<DeviceType>& kmcdSurf)
  {
    ...
    const ordinal_type m = Impl::IsothermalTransientContStirredTankReactor_Problem
...
//
    ...
    if (m < 128) {
      ...
      work_size = Impl::IsothermalTransientContStirredTankReactor
