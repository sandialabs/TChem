...
#define DASHLINE(file)							\
  ...
  kmd_type_1d_view_host
  KineticModelData::clone(const int n_models) {
...
#define DASHLINE(file)							\
  ...
  modifyArrheniusForwardParameter(const kmd_type_1d_view_host kmds,
    ...
    if (kmds.span() > 0 && reac_indices.span() > 0 && factors.span()) {
      ...
      TCHEM_CHECK_ERROR(n_reac_indices > kmds(0).nReac_, "Error: reac indices array has a larger number of indices than nReac");
    } else {
    ...
    Kokkos::parallel_for
      (Kokkos::RangePolicy<host_exec_space>(0, n_models), [=](const int p) {
	auto& kmd = kmds(p);
  ...
  modifyArrheniusForwardParameter(const kmd_type_1d_view_host kmds,
    ...
    real_type_3d_view_host factors(do_not_init_tag("factors"), n_model, n_reac_indices, 3);
    Kokkos::parallel_for
      (Kokkos::RangePolicy<host_exec_space>(0, n_model),
       [=](const ordinal_type imodel) {
	 auto pre_exponential_factor =
	   parameters["modifier_pre_exponential_No_"+std::to_string(imodel)];
	 auto temperature_coefficient_factor =
	   parameters["modifier_temperature_coefficient_No_"+std::to_string(imodel)];
	 auto activation_energy_factor =
	   parameters["modifier_activation_energy_No_"+std::to_string(imodel)];
	 for (ordinal_type ireac = 0; ireac < n_reac_indices;++ireac) {
