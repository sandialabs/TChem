...
#define TCHEM_RUN_TRANSIENT_CONT_STIRRED_TANK_REACTOR_SACADO_JACOBIAN() \
  TransientContStirredTankReactorSacadoJacobian_TemplateRun(            \
                                                            profile_name, \
                                                            value_type(), \
                                                            policy,     \
                                                            state,      \
  ...
  void
  TransientContStirredTankReactorSacadoJacobian::runDeviceBatch( /// input
  ...
  {
    const std::string profile_name ="TChem::TransientContStirredTankReactorSacadoJacobian::runDeviceBatch";
    using problem_type = Impl::TransientContStirredTankReactor_Problem<real_type, device_type>;
    const ordinal_type m = problem_type::getNumberOfEquations(kmcd, kmcdSurf);
    
    if (m < 16) {
      using value_type = Sacado::Fad::SLFad<real_type,16>;
      TCHEM_RUN_TRANSIENT_CONT_STIRRED_TANK_REACTOR_SACADO_JACOBIAN();
    } else if  (m < 32) {
  ...
  void
  TransientContStirredTankReactorSacadoJacobian::runHostBatch( /// input
  ...
  {
    const std::string profile_name ="TChem::TransientContStirredTankReactorSacadoJacobian::runHostBatch";
    using problem_type = Impl::TransientContStirredTankReactor_Problem<real_type, host_device_type>;
    const ordinal_type m = problem_type::getNumberOfEquations(kmcd, kmcdSurf);
