...
namespace Impl {
  ...
  struct IgnitionZeroD_Problem
  {
    ...
    static ordinal_type getWorkSpaceSize(const kinetic_model_type& kmcd)
    {
      ...
      = SourceTerm<value_type, device_type>::getWorkSpaceSize(kmcd);
      ...
      const ordinal_type workspace_size =
        (workspace_size_analitical_jacobian >
          workspace_size_sacado_numerical_jacobian ? workspace_size_analitical_jacobian
          : workspace_size_sacado_numerical_jacobian);
