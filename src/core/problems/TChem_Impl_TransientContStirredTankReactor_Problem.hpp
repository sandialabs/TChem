...
struct TransientContStirredTankReactor_Problem
{
  ...
  {
    return kmcd.nSpec +  1 + kmcdSurf.nSpec - cstr.number_of_algebraic_constraints;
  }
  ...
  {
    return kmcd.nSpec +  1 + kmcdSurf.nSpec;
  }
    ...
    const kinetic_surf_model_type& kmcdSurf)
  {
    ...
    const ordinal_type src_workspace_size = TransientContStirredTankReactorRHS<value_type, device_type>
           ...
           const value_type_1d_view_type& f) const
  {
    ...
    const value_type density = _cstr.pressure * Wmix/ _kmcd.Runiv / t ;

    const value_type_1d_view_type siteFraction =
    Kokkos::subview(x,range_type(_kmcd.nSpec + 1, _kmcd.nSpec + 1 +_kmcdSurf.nSpec  ));

    TransientContStirredTankReactorRHS<value_type, device_type>::team_invoke_sacado(member,
                                                ...
                                                const real_type_1d_view_type& f) const
  {
    ...
    const real_type_1d_view_type siteFraction(&x(_kmcd.nSpec+1), _kmcdSurf.nSpec);
      //compute density
    ...
    TransientContStirredTankReactorRHS<real_type, device_type>::team_invoke(member,
                                             t, // constant temperature
                                             Ys,
                                             siteFraction,
                                             density,
