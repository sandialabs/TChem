...
int main(int argc, char *argv[]) {
  ...
  bool run_source_term(true), run_analytic_Jacobian(true), run_numerical_Jacobian(true),
      run_numerical_Jacobian_fwd(true), run_sacado_Jacobian(true);
  ...
  //
  opts.set_option<std::string>("output-file-times", "Wal times file name e.g., times.dat", &outputFileTimes);
  opts.set_option<int>("league_size", "league size ", &league_size);
  opts.set_option<bool>("useYaml", "If true, use yaml to parse input file", &useYaml);
  ...
  //
  opts.set_option<bool>("use-sample-format", "If true, input file does not have header or format", &use_sample_format);
  opts.set_option<bool>("use-shared-workspace", "If true, it use team policy shared scratch memory for workspace",
                        &use_shared_workspace);
  opts.set_option<bool>("verbose", "If true, printout the first omega values", &verbose);
  opts.set_option<bool>("run_source_term", "If true, runs source term computation", &run_source_term);
  opts.set_option<bool>("run_analytic_Jacobian", "If true, runs analytic Jacobian computation", &run_analytic_Jacobian);
  opts.set_option<bool>("run_numerical_Jacobian",
                        "If true, runs numerical(Richardson Extrapolation) Jacobian computation",
                        &run_numerical_Jacobian);
  //
  opts.set_option<bool>("run_numerical_Jacobian_fwd",
                        "If true, runs numerical (Forward Difference) Jacobian computation",
                        &run_numerical_Jacobian_fwd);
  opts.set_option<bool>("run_sacado_Jacobian", "If true, runs analytic, sacado,  Jacobian computation",
                        &run_sacado_Jacobian);
  ...
  Kokkos::initialize(argc, argv);
  {
    ...
    if (run_numerical_Jacobian) {
      printf(
          "Running numerical-Jacobian (Richardson Extrapolation) computations using: team_size %d vector_size %d... \n",
          team_size, vector_size);
    ...
    if (run_numerical_Jacobian_fwd) {
      ...
      if (verbose) {
        ...
        Kokkos::deep_copy(numerical_Jacobian_fwd_host, numerical_Jacobian_fwd);
        {
          ...
          auto numerical_Jacobian_fwd_host_at_0 =
              Kokkos::subview(numerical_Jacobian_fwd_host, 0, Kokkos::ALL(), Kokkos::ALL());
    ...
    printf("Testing Arguments: \n batch size %d\n jac dimension %d\n chemfile %s\n thermfile %s\n inputfile %s\n "
           "outputfile %s\n verbose %s\n",
           nBatch, (kmcd.nSpec + 1), chemFile.c_str(), thermFile.c_str(), inputFile.c_str(), outputFile.c_str(),
           verbose ? "true" : "false");
  }
  Kokkos::finalize();
