...
// #include "TChem_ReactionRatesSurface.hpp"
...
main(int argc, char* argv[])
{
  ...
  // omega.dat", &outputFile); opts.set_option<std::string>("outputfileGasSurf",
  // "Output omega gas from surface file name e.g., omegaGasSurf.dat",
  // &outputFileGasSurf); opts.set_option<std::string>("outputfileSurf", "Output
  // omega file name e.g., omegaGasSurf.dat", &outputFileSurf);
  opts.set_option<int>(
    "batchsize",
  ...
  Kokkos::initialize(argc, argv);
  {
    ...
    real_type_2d_view kfor("ForwardConstantSurface", nBatch, kmcdSurf.nReac);
    real_type_2d_view krev("ReverseConstantSurface", nBatch, kmcdSurf.nReac);
    ...
    TChem::KForwardReverseSurface::runDeviceBatch(nBatch,
                                                  // inputs
                                                  state,
                                                  gk,
                                                  gkSurf,
    ...
    if (verbose) {
      ...
      /// all values are same (print only the first one)
      {
        ...
        TChem::Test::writeReactionRates(
          outputFilekrev, kmcd.nReac, krev_host_at_0);

        auto kfor_host_at_0 = Kokkos::subview(kfor_host, 0, Kokkos::ALL());
        TChem::Test::writeReactionRates(
          outputFilekfor, kmcd.nReac, kfor_host_at_0);
