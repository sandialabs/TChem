...
main(int argc, char* argv[])
{
  ...
  std::string outputFile(prefixPath + "plugflowreactorRHS.dat");
  ...
  opts.set_option<std::string>(
    "outputfile",
    "Output rhs file name e.g., plugflowreactorRHS.dat",
    &outputFile);
  opts.set_option<int>(
    "batchsize",
  ...
  Kokkos::initialize(argc, argv);
  {
    ...
    const auto nEquations = kmcd.nSpec + kmcdSurf.nSpec + 3;
    real_type_2d_view rhs("plugflowreactorRHS", nBatch, nEquations);
    ...
    TChem::PlugFlowReactorRHS::runDeviceBatch(nBatch,
                                              // inputs
                                              state,        // gas
