...
#include "TChem_Impl_RateOfProgress_Problem.hpp"
...
main(int argc, char* argv[])
{
  ...
  Kokkos::initialize(argc, argv);
  {
    ...
    using problem_type = TChem::Impl::RateOfProgess_Problem<fad_type, host_device_type>;
    ...
    const int n = problem.getNumberOfVariables();
    ...
    problem.computeJacobianSacado(member, x, J_s);

    Tines::showMatrix("SacadoJacobian", J_s);

    /// numeric tests
    auto compareJacobian = [m, n](const std::string &label, auto &A, auto &B) {
...

    {
      ...
      //
      real_type_2d_view_type J_a("J_analitical",  m, n);
      ...
      Tines::showMatrix("AnaliticalJacobian", J_a);
